---
layout: default
title: Other vim-go tutorials
nav_exclude: true
parent: Golang
grand_parent: Programming Languages
permalink: /docs/languages/golang/other-vim-go-tutorial
---

:: __[back]({% link docs/languages/golang/other.md %})__ :: [how to debug with `vim-go`]({% link docs/languages/golang/debug.md %}#debuging-go-with-vim-go) ::

# vim-go-tutorial

Tutorial for vim-go [^1]. A simple tutorial on how to install and use vim-go.
...
One of the most noticeable changes in the ecosystem since this tutorial was written is the introduction of Go modules. The tutorial does not yet explain some of the differences in behavior of the go tool and other community provided tools in [GOPATH mode vs module aware mode](https://golang.org/cmd/go/#hdr-Module_support). The vast majority of vim-go features work as described here with the exception of commands that rely on `guru`. `guru` _only_ works as expected in [GOPATH mode](https://golang.org/cmd/go/#hdr-Module_support). Other commands may require the use of `gopls`, and many options have been added to vim-go to control whether `gopls` or the traditional tool is used under the hood. Please reference vim-go's help (`:help vim-go`) when in doubt.
...
* [fatih](https://github.com/fatih/vim-go) also use these shortcuts to build and run a Go program with `<leader>b` and
`<leader>r`:
...
* By default the leader shortcut is defined as: `\` [fatih](https://github.com/fatih/vim-go) mapped his leader to
`,` as [fatih](https://github.com/fatih/vim-go) find it more useful with the following setting (put this in the
beginning of .vimrc):
...
[motion](https://github.com/fatih/motion) under the hood. [fatih](https://github.com/fatih/vim-go) wrote motion
...
If you are interested in learning more about `motion`, check out the blog post [fatih](https://github.com/fatih/vim-go) wrote for
...
Let [fatih](https://github.com/fatih/vim-go) show some of the snippets that he's using the most. Change your `main.go`
content to:
...
Great. Now let [fatih](https://github.com/fatih/vim-go) show one last snippet that he thinks is very useful. As you see
...
* A lot of people ask for [fatih](https://github.com/fatih/vim-go) colorscheme. He's using a slightly modified
...
vim-go has several ways of improving navigation. First let [fatih](https://github.com/fatih/vim-go)
...
In our `Edit it` section [fatih](https://github.com/fatih/vim-go) mentioned a tool called `motion`, which is a
...
These commands are very productive depending on how you use them, so [fatih](https://github.com/fatih/vim-go) think
...
which will output (e.g. `$GOPATH` is set to `~/Code/Go`):
...
`Vim-go` supports two tools to rename identifiers: `gorename` and `gopls`. It currently uses `gorename` by default.

You can set which tool is used by configuring `g:go_rename_command` (see `:help g:go_rename_command`).
`gorename` only works in GOPATH mode. `gopls` can be used to rename in both GOPATH mode and module aware mode and will be the default tool used to rename soon.

Let us change `main.go` to the following content:
...
This tutorial was created by [fatih](https://github.com/fatih/vim-go) in his spare time. If you like it and would
like to donate, you now you can be a fully supporter by [being a
patron](https://www.patreon.com/fatih)! 

By being a patron, you are enabling vim-go to grow and mature, helping [fatih](https://github.com/fatih/vim-go) to
...
[^1]: [vim-go tutorial](https://github.com/fatih/vim-go/wiki/Tutorial) by [_fatih_](https://github.com/fatih/vim-go)
